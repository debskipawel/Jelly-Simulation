...
D11Buffer::D11Buffer(const D11Device& device, const void* data, D11BufferDesc desc)
    : m_desc(desc), m_device(device.Raw()), m_context(device.Context().Get())
{
    CreateBuffer(data, m_desc);
}

void D11Buffer::Update(const void* data, size_t size)
{
...
void D11Buffer::CreateBuffer(const void* data, const D11BufferDesc& desc)
{
